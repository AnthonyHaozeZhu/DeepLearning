...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(28 * 28, 400)  # 全连接层，图片像素是28 * 28, 所以输入也是28 * 28, 防止计算量太大映射到50，weight [28 * 28, 50] bais[50, ]
        self.fc1_drop = nn.Dropout(0.2)  # 防止过拟合，在训练时随机以一定的概率丢掉一些数据，丢掉20%的数据（可以加激活函数）
        self.fc2 = nn.Linear(400, 80)  # 全连接层
        self.fc2_drop = nn.Dropout(0.2)
        self.fc3 = nn.Linear(80, 50)  # 最后有十个类型（1-10），所以将最终的结果映射到10个维度上
    ...
    def forward(self, x):
        ...
        x = self.fc3_drop(x)
        x = self.fc4(x)
        return F.log_softmax(x, dim=1)  # log_softmax_log(p), dim = 1是每张图片对第一个维度做softmax
...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.8)  # 优化器, lr是学习率，momentum动量
